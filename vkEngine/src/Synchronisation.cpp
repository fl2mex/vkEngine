...
namespace vkEngine
{
	vk::Semaphore vkEngine::CreateSemaphore(const vk::Device device, const bool debug)
	{
		vk::SemaphoreCreateInfo createInfo{};
		createInfo.flags = vk::SemaphoreCreateFlags();
		try
		{
			const vk::Semaphore semaphore = device.createSemaphore(createInfo);
	...
	vk::Fence vkEngine::CreateFence(const vk::Device device, const bool debug)
	{
		vk::FenceCreateInfo createInfo{};
		createInfo.flags = vk::FenceCreateFlags() | vk::FenceCreateFlagBits::eSignaled;
		try
		{
			const vk::Fence fence = device.createFence(createInfo);
